...
## Leitura da etiqueta pela câmera

Foi adicionada a opção **Ler etiqueta com a câmera**, específica para a etiqueta do modelo mostrado na segunda foto, cujo patrimônio segue o padrão `520000XXXX`.

O leitor utiliza ZXing no navegador para ler o código de barras. A leitura não confirma automaticamente o patrimônio: após localizar o tombo, o usuário ainda precisa clicar em **Confirmar encontrado**.

A câmera precisa ser usada em um contexto seguro, como HTTPS (por exemplo, GitHub Pages) ou `localhost`, e o navegador precisa receber permissão para acessar a câmera.
